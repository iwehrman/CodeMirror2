...
window.CodeMirror = (function() {
  ...
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
    ...
    var j = from, fixedPos = compensateForHScroll(display);

    cm.view.doc.iter(from, to, function(line) {
      if (nextIntact && nextIntact.to == j) nextIntact = intact.shift();
      if (!nextIntact || nextIntact.from > j)
        display.lineDiv.insertBefore(buildLineElement(cm, line, j, fixedPos), curNode);
      else
        curNode = curNode.nextSibling;
      ++j;
    });
  ...
  function buildLineElement(cm, line, lineNo, fixedPos) {
    if (line.hidden && (!line.hidden.handle.showWidgets || !line.widgets))
      return elt("div");
    ...
    if (cm.options.lineNumbers || markers) {
      ...
      if (cm.options.lineNumbers)
        gutterWrap.appendChild(elt("div", lineNumberFor(cm.options, lineNo),
                                   "CodeMirror-linenumber CodeMirror-gutter-elt",
                                   "left: " + display.lineGutter.offsetLeft + "px; width: "
        ...
        for (var k = 0; k < cm.options.gutters.length; ++k) {
          ...
          if (found) {
            ...
            gutterWrap.appendChild(elt("div", [found], "CodeMirror-gutter-elt", "left: " +
                                       gutterElt.offsetLeft + "px; width: " + gutterElt.clientWidth + "px"));
          }
        }
    }
    ...
    if (line.widgets)
      for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
        ...
        if (widget.noHScroll) {
          ...
          var width = display.wrapper.clientWidth;
          node.style.left = fixedPos + "px";
          if (!widget.coverGutter) {
            width -= display.gutters.offsetWidth;
            node.style.paddingLeft = display.gutters.offsetWidth + "px";
          }
  ...
  function highlightWorker(cm) {
    ...
    doc.iter(doc.frontier, Math.min(doc.size, cm.display.showingTo + 500), function(line) {
  ...
  function unredoHelper(cm, from, to, dir) {
    var doc = cm.view.doc, hist = cm.view.doc.history;
  ...
  function undo(cm) {
    var hist = cm.view.doc.history;
    unredoHelper(cm, hist.done, hist.undone, -1);
  }
  function redo(cm) {
    var hist = cm.view.doc.history;
    unredoHelper(cm, hist.undone, hist.done, 1);
  }
  ...
  CodeMirror.prototype = {
    ...
    isDirty: function () {return (this.view.doc.history.dirtyCounter != 0);},

    markClean: function() {this.view.doc.history.dirtyCounter = 0;},
    ...
    markText: operation(null, function(from, to, className, options) {
      ...
      doc.history.addChange(from.line, old.length, old, true);
      return marker;
    }),
    ...
    setBookmark: function(pos) {
      ...
      line.markedSpans = (line.markedSpans || []).concat([span]);
      marker.lines.push(line);
      doc.history.addChange(pos.line, 1, [old]);
      return marker;
    },
    ...
    moveV: operation(null, function(dir, unit) {
      ...
      else if (dir < 0 && cur.line > 0 && (line = getLine(doc, target.line)).widgets) {
        var above, below;
        for (var i = 0; i < line.widgets.length; ++i)
          if (line.widgets[i].above) above = true; else below = true;
        if (below && target.ch == line.text.length)
          target = coordsChar(this, x, cursorCoords(this, {line: target.line, ch: line.text.length}, "div").bottom - 3);
        else if (above && posEq(target, cur))
          target = coordsChar(this, x, heightAtLine(doc, target.line) - 3);
      }
  ...
  TextMarker.prototype.clear = function() {
    ...
    if (min != null) {
      ...
      this.cm.view.doc.history.addChange(min, old.length, old, true);
    }
    this.lines.length = 0;
  ...
  function removeMarkedSpan(spans, span) {
    for (var r, i = 0; i < spans.lenght; ++i)
